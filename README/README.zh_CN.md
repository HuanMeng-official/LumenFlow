...
LumenFlow（中文名：流光）是一款使用 Flutter 构建的现代化 AI 聊天应用，在 Android、Windows 和 Linux 平台上提供无缝的对话体验。支持 12+ 个 AI 服务提供商，使用 SQLite 数据库进行本地数据持久化，具备丰富的多模态处理能力，提供多功能的 AI 助手体验。
...
### 核心 AI 能力
- **12+ AI 平台支持**: OpenAI、Claude、Google Gemini、DeepSeek、硅基流动、MiniMax、智谱 AI、Kimi、LM-Studio（本地部署）、Grok、OpenRouter 和其他（自定义 OpenAI 兼容 API）
...
- **文件附件**: 上传和提取各种文件类型的内容
- **用户资料**: 个性化设置，支持头像、用户名、性别和生日自定义
- **角色卡生成器**: 内置的基于网页的角色卡生成器，用于创建详细的AI人物设定
...
│   └── http_server_service.dart # 角色卡生成器的 HTTP 服务器服务
├── providers/                # AI 提供商实现
...
│   ├── grok_provider.dart     # Grok (xAI) 实现
│   ├── openrouter_provider.dart # OpenRouter 实现
...
- **多平台支持**: 在单个应用程序中配置和管理 12+ 个 AI 平台
...
11. **OpenRouter**
    - API 端点：`https://openrouter.ai/api/v1/responses`
    - 支持的模型：来自多个提供商的各种模型
    - OpenAI 兼容的 API 格式
